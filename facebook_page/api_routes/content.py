...
def simulate_pipeline(db: Session):
    ...
    db.add(AuditLog(action="Trend Aggregation", details={"status": "success", "step": "trending", "reason": "Found high-virality topics in B2B SaaS Community."}))
    db.commit()
    time.sleep(3)
    
    db.add(AuditLog(action="Opportunity Detection", details={"status": "success", "step": "opportunity", "reason": "Matched 'Remote Work Productivity' trend with brand persona."}))
    db.commit()
    time.sleep(2)
    
    db.add(AuditLog(action="Content Generation", details={"status": "success", "step": "generation", "reason": "Drafted variants applying Community Engagement Poll #3."}))
    db.commit()
    time.sleep(3)
