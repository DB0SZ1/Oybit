...
class SchedulerQueue:
    ...
    def get_due_jobs_for_twilio(self) -> list[dict]:
        ...
        try:
            ...
            # Find jobs that are within 30 mins, pending, and not notified yet
            jobs = db.query(SchedulerJob).join(Post, SchedulerJob.post_id == Post.id).filter(
                SchedulerJob.scheduled_at <= target_time,
                SchedulerJob.status == "pending",
                Post.twilio_notified == False
            ).order_by(asc(SchedulerJob.scheduled_at)).all()
