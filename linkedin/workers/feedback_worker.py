...
from feedback_loop.learning_engine import analyze_patterns
from feedback_loop.persona_patcher import apply_persona_patches
from feedback_loop.mirofish_refiner import build_refinement_signal
from feedback_loop.archiver import archive_old_logs
...
def record_heartbeat():
    try:
        ...
        if not hb:
            hb = WorkerHeartbeat(worker_name="feedback_worker", status="running", last_heartbeat=datetime.utcnow())
            db.add(hb)
        else:
            hb.status = "running"
