...
def simulate_pipeline(db: Session):
    ...
    db.add(AuditLog(action="Trend Aggregation", details={"status": "success", "step": "trending", "reason": "Found high-virality topics in Personal Branding."}))
    db.commit()
    time.sleep(3)
    
    db.add(AuditLog(action="Opportunity Detection", details={"status": "success", "step": "opportunity", "reason": "Matched 'Founder Journey' trend with brand persona."}))
    db.commit()
    time.sleep(2)
    
    db.add(AuditLog(action="Content Generation", details={"status": "success", "step": "generation", "reason": "Drafted variants applying Storytelling Arc #1."}))
    db.commit()
    time.sleep(3)
