...
# Try to load envs
load_dotenv(os.path.join(os.getcwd(), 'linkedin', '.env'))
...
# Where to append the public log. Add more paths if you want it duplicated.
BUILD_LOG_PATHS = [
    os.path.join(os.getcwd(), "linkedin", "BUILD_LOG.md"),
    os.path.join(os.getcwd(), "hf_deploy", "BUILD_LOG.md"),
    os.path.join(os.getcwd(), "BUILD_LOG.md")
]

if not OPENROUTER_API_KEY:
    print("Error: OPENROUTER_API_KEY not found in .env files.")
    exit(1)
    ...
    for path in BUILD_LOG_PATHS:
        if os.path.exists(path):
            ...
            with open(path, "w", encoding="utf-8") as f:
                f.write(content)
            print(f"[*] Updated build log at {path}")
...
def main():
    ...
    def bind_dummy_port():
        try:
            ...
            s.bind(('127.0.0.1', 10323))
            s.listen(1)
            print("[*] Bound dummy port 10323 for process management.")
            while True:
                conn, addr = s.accept()
                conn.close()
        except Exception as e:
            print(f"[!] Failed to bind port 10323: {e}. Is it already running?")
            exit(1)
            
    threading.Thread(target=bind_dummy_port, daemon=True).start()
